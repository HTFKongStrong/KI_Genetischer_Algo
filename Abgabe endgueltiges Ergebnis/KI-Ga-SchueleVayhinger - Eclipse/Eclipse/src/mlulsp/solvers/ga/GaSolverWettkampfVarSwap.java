...
public class GaSolverWettkampfVarSwap {
    //Eigens zusetzende Entscheidungsvariablen: nur zu Beginn, entwickeln dann Eigenleben
    private final int populationsGroesse = 250; //NUR bis 500, sonst exception; da Nullpointer muss durch AnzahLösungen teilbar sein
    private final int selektionsDruck = 8; //ab 4!!!! //replaceWettkampf:  wie viele in den matingpool gelangen sollen, also die hoechste Anzahl an Eltern die reproduzieren koennen
    ...
    private final double prozentVariationPmut = 0.03; // 0,0 - 1,0 // ab welchem Verhaeltnis von anzahlLoesungen soll pMut erhoeht werden
    ...
    public GaSolverWettkampfVarSwap(int anzahlLoesungen) {
        ...
        while(anzahlIndividuenGes < anzahlLoesungen){ // es duerfen nur 400.000 Individuuen pro Optimierungslauf erstellt werden
            ...
            while(sizePopKids < populationsGroesse && anzahlIndividuenGes < anzahlLoesungen){ //beenden wenn gleiche Populationsgroesse erreicht ist
                ...
                if(wechsel && (generation%2==0)) {
                    Mutation.swapMutationVar(kids[0], pMutVar);
                    Mutation.swapMutationVar(kids[1], pMutVar);
                }
